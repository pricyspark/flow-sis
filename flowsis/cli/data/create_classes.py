...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Read video manifest to create class mapping."
    )
...
def create_json(
    csv_path: Path, json_path: Path, class_cols: Iterable[int | str]
) -> None:
    ...
    with open(csv_path, newline="") as file:
        ...
        for row in reader:
            components = [
                ...
                if (
                    value := (
                        row[col] if isinstance(col, int) else row[header.index(col)]
                    )
                )
            ]
