...
def get_object_records(example: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    if not isinstance(objects, list):
        raise TypeError(f"Expected example['objects'] to be a list, received {type(objects).__name__}.")
...
def get_object_feature_schema(objects_feature: Any) -> Any:
    ...
    if isinstance(schema, list):
        if len(schema) != 1:
            raise TypeError(f"Expected a single object feature schema, received {len(schema)} entries.")
        return schema[0]
    return schema
    ...
    *,
    ...
    if object_index is not None:
        if object_index < 0 or object_index >= len(records):
            raise IndexError(f"Object index {object_index} is out of bounds for {len(records)} objects.")
